...
# Copy sources
WORKDIR $GOPATH/src/github.com/mjording/oauth2-proxy
...
COPY --from=builder /go/src/github.com/mjording/oauth2-proxy/oauth2-proxy /bin/oauth2-proxy
COPY --from=builder /go/src/github.com/mjording/oauth2-proxy/jwt_signing_key.pem /etc/ssl/private/jwt_signing_key.pem
